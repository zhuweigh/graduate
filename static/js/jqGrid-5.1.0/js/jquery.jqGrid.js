...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		$("<div "+getstyle(ts.p.styleUI+'.common','overlay', false, 'jqgrid-overlay')+ " id='lui_"+this.id+"'></div>").insertBefore(gv);
                ...
                $("<div "+getstyle(stylemodule,'loadingBox', false, 'loading')+" id='load_"+this.id+"'></div>").insertBefore(gv);

                //update loading
                $("#load_"+this.id).append('<div class="la-ball-spin-clockwise la-2x"></div>');
                for (var i_la_ball = 0; i_la_ball < 8; i_la_ball++) {
                    $('.la-ball-spin-clockwise').append('<div></div>');
                };
		...
		beginReq = function() {
			...
			$(ts).jqGrid("progressBar", {method:"show", loadtype : ts.p.loadui, htmlcontent: ''});
		},
		endReq = function() {
